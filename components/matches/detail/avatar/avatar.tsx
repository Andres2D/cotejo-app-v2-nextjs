...
const AvatarMatchLayout: NextPage<Props> = ({
  id,
  name,
  overall,
  image,
  ...
  const selectPlayerHandler = () => {
    ...
    if (
      details.playersSelected.length > 0 &&
      details.playersSelected[0].playerId !== id
    ) {
  ...
  return (
    <div className={`${styles.container} ${className}`}>
      <div className={styles.avatarContainer} onClick={selectPlayerHandler}>
        <Popover isOpen={details.playersSelected[0]?.playerId === id}>
          <PopoverTrigger>
            <Avatar
              size="lg"
              name={name}
          ...
          <PopoverContent bg='blue.400' className={styles.avatarActions}>
            <RepeatIcon 
              w={8} 
              h={8}
              color={'gray.50'}
        ...
        {showNamesOption && <h5 className={styles.playerName}>{name}</h5>}
        {showStatsOption && <h5 className={styles.playerOverall}>{overall}</h5>}
      </div>
    </div>
  );
};
